...
krill_fun_norv <- function (pars, pars_sens=0, lg_ini=0) {
  ...
  if (sum(unlist(pars_sens))==0){
    lg_ind[start,] <- round( runif(nb_ind, min(20, na.rm=TRUE), max(40, na.rm=TRUE)), digits=2) 
  } else {
    lg_ind[start,]     <- lg_ini
  }
  ...
  cp_mez  <- 0.69 + 0*cw_ind # T. raschii
  ...
  for (t in time[2:length(time)]) {
    ...
    if ( length(j) != 0 ) {
      ...
      for (k in j) {
        ...
        if ( mature[t,k] & nb_moult[k]%%2==0 & food[1,t]>food_bloom ) {
